...
public class DatatypeHelper {
  ...
  private static URI ensureResolved(String termUri) {
    ...
    if (uri.matches("\\w+:.+")) {
      ...
      String base = getNamespaceURIForPrefix(candidatePfx).orElse(null);
      if (base != null) {
  ...
  public static QualifiedIdentifier name(final String n) {
    ...
    String uri = getNamespaceURIForPrefix(pfx).orElse("");

    return new QualifiedIdentifier().withQName(new QName(uri, name, pfx));
  }

  private static Optional<String> getNamespaceURIForPrefix(String pfx) {
