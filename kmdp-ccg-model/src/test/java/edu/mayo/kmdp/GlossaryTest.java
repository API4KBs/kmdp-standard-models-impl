...
import edu.mayo.kmdp.ccg.model.ObjectFactory;
...
class GlossaryTest {
  ...
  void testGeneratedModel() {
    ...
    GlossaryEntry ge = new GlossaryEntry()
        .withPropositionalConcept(hypertense)
        .withDef(
            new OperationalDefinition()
                .withId(UUID.randomUUID().toString())
                .withFocalConcept(hypertension)
                .withPropositionalConcept(hypertense)
                .withName("Hypertension")

                .withSemanticDef(
                    new KnowledgeResourceRef()
                        .withResourceId(SemanticIdentifier
                            .newId(URI.create("http://test.ckm.mock.edu/123456#1")))
                        .withName("NLM Definition of HT")
                        .withDescription("Hypertension is.."))
                .withDefault(true));

    ObjectFactory of = new ObjectFactory();
    Optional<String> x = JaxbUtil.marshall(Collections.singleton(of.getClass()),
        ge,
        of::createGlossaryEntry,
    ...
    Optional<Schema> schema = XMLUtil
        .getSchemas(
            GlossaryTest.class.getResource("/xsd/ClinicalConceptGlossary/model/model.xsd"),
            catalogResolver(
                "/xsd/km-surrogate-catalog.xml"));
    assertTrue(schema.isPresent());
    assertTrue(XMLUtil.validate(x.get(), schema.get()));

    assertTrue(x.get().contains(hypertense.getReferentId().toString()));
    assertTrue(x.get().contains("http://snomed.info/sct"));

    JSonUtil.printOutJson(ge);
  }
  ...
  void testGlossaryDefinitionAssets() {
    ...
    KnowledgeAsset mcbt_diabetes_vs = new KnowledgeAsset()
        ...
        .withAssetId(
            SemanticIdentifier.newId(URI.create("http://terms.mayo.edu/valueset/123456789")))
    ...
    KnowledgeAsset def1 = new KnowledgeAsset()
        .withAssetId(SemanticIdentifier.newId(URI.create("http://test.ckm.mock.edu/9999-2")))
    ...
    KnowledgeAsset def2 = new KnowledgeAsset()
        .withAssetId(SemanticIdentifier.newId(URI.create("http://test.ckm.mock.edu/9999-1")))
